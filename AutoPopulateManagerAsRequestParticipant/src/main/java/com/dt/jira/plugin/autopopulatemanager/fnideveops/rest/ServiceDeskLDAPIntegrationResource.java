package com.dt.jira.plugin.autopopulatemanager.fnideveops.rest;
 ...
 *
 */
@Path("/fildapintegration")
@Named("ServiceDeskLDAPIntegrationResource")
...
public class ServiceDeskLDAPIntegrationResource {
		...
		ServiceDeskLDAPIntegrationResourceModel myRestResourceModel=new ServiceDeskLDAPIntegrationResourceModel();		   
		   try {
				...
				if (acc == 0) {
					...
					logger.debug("-------------------managernamefield-------------"+managernamefield);
    ...
    private void setConfiguration(){
		...
		String baseDn = (String) pluginSettings.get(PLUGIN_STORAGE_KEY + ".ldapBaseDn");
   		...
   		logger.debug("initCtx: "+ initCtx);
   		logger.debug("srvrName: "+srvrName);
   		logger.debug("baseDn: "+baseDn);
   		logger.debug("username: "+username);

   		logger.debug("Set plugin configuration");
		...
		public Response get(@Context HttpServletRequest request)
		{
		  ...
		  return Response.ok(transactionTemplate.execute(new TransactionCallback()
		  {
		    public Object doInTransaction()
		    {
		      ...
		      logger.debug("************* " + ldapSrvc.getLdapUid());
		      return ldapSrvc;
		    }
		  })).build();
		}
		...
		public Response put(final LdapSrvc ldapSrvc, @Context HttpServletRequest request)
		{
		  ...
		  transactionTemplate.execute(new TransactionCallback()
		  {
		    public Object doInTransaction()
		    {
		      ...
		      logger.debug(LdapSrvc.class.getName() + ".ldapInitCtx"+ ldapSrvc.getLdapInitCtx());
		      logger.debug(LdapSrvc.class.getName() + ".ldapSrvrName"+ ldapSrvc.getLdapSrvrName());
		      logger.debug(LdapSrvc.class.getName() + ".ldapBaseDn"+ ldapSrvc.getLdapBaseDn());
		      logger.debug(LdapSrvc.class.getName() + ".ldapUid"+ ldapSrvc.getLdapUid());
