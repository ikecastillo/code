...
/**
 * REST service that provides the data for Number Of Deployments Bar chart.
 *
 */
@Path("/numberofdeployments")
public class NumberOfDeployments {
    /* Logger */
    private final Logger logger = Logger.getLogger(NumberOfDeployments.class);
    ...
    public Response getMessage(@QueryParam("searchFilter") String searchFilter,
        ...
        if (sevtMajor.isValid()) {
            ...
            if(issues!= null && issues.size() > 0)  {
                ...
                updateHashMapBasedOnLegend(legendCF, issues, legendHashMap, xAxisOnBarChartCF);
    ...
    private void updateHashMapBasedOnLegend(CustomField legendCF, List<Issue> issues, Map<String,
            List<Issue>> legendMap, CustomField xAxisOnBarChartCF) {
        ArrayList<Issue> newArry;
        String legendFieldType = null;
        if (legendCF!= null) {
        ...
        /*
            Filter List used to filter the issues list to what we need to plot.
            This list will have entries of form Date+changeType
         */
        List<String> filterList = new ArrayList<String>();
        List<Issue> filteredIssuesList = new ArrayList<Issue>();

        //First Filter out the issues to be plotted in the filteredIssuesList using filterList
        for (Issue issue : issues) {
            LazyLoadedOption singleChoiceListOptionVal = (LazyLoadedOption) issue.getCustomFieldValue(legendCF);
            Date dateCustField = (Date)issue.getCustomFieldValue(xAxisOnBarChartCF);
            if (dateCustField != null) {
                String dateVal = df.format(dateCustField);
                String changeTypeOfIssue = singleChoiceListOptionVal.getValue();
                if(!filterList.contains(dateVal + changeTypeOfIssue)) {
                    filterList.add(dateVal+changeTypeOfIssue);
                    filteredIssuesList.add(issue);
                }
            }
        }

        //Then create the required hashmap based on the filtered issues list
        for(Issue issue : filteredIssuesList) {
            LazyLoadedOption singleChoiceListOptionVal = (LazyLoadedOption)issue.getCustomFieldValue(legendCF);
    ...
    /**
     * Helper method that extracts the field value of 'Incident Start' or any Date field
