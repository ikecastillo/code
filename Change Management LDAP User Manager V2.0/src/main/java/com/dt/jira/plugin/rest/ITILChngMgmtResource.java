 ...
 */
@Path("/message")
public class ITILChngMgmtResource {

	private final Logger logger = Logger.getLogger(ITILChngMgmtResource.class);
	...
	public ITILChngMgmtResource(UserManager userManager, PluginSettingsFactory pluginSettingsFactory, TransactionTemplate transactionTemplate)
	  {
    ...
    { 	   
		...
		ITILChngMgmtResourceModel myRestResourceModel=new ITILChngMgmtResourceModel();		   
		   try {
				...
				if (acc == 0) {
					myRestResourceModel=(ITILChngMgmtResourceModel) accDetails.get(2);
					...
					logger.info("LDAP UserProfile User Reporter's Title : "+myRestResourceModel.getReporterTitle());
				} 
					...
					CustomField  deptCustomField = customFieldManager.getCustomFieldObjectByName("Reporter's Department");
					String deptfield = deptCustomField.getHiddenFieldId();

					logger.info("-------------------deptfield-------------"+deptfield);
					responseMap.put(deptfield, myRestResourceModel.getDeptName());
					CustomField  managerNameCustomField = customFieldManager.getCustomFieldObjectByName("Manager's Name");
					...
					responseMap.put(managernamefield, myRestResourceModel.getMngrAccountName());
			   		CustomField  reporterTitleCustomField = customFieldManager.getCustomFieldObjectByName("Reporter's Title");
			  		String reportertitlefield = reporterTitleCustomField.getHiddenFieldId();

			   		logger.info("-------------------reportertitlefield-------------"+reportertitlefield);
			   		responseMap.put(reportertitlefield, myRestResourceModel.getReporterTitle());
		...
		public List getUserAccountDetailsByMailId(String userMailId) {
			...
			try {
					...
					if (namingenumeration != null && namingenumeration.hasMoreElements()) {
						...
						myRestResourceModel.setReporterTitle((String) getAttribsForKey(searchresult, as[9]).get(0));
