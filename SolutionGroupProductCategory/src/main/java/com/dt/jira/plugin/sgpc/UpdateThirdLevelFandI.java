 ...
 */
@Path("/FandIUpdate")
public class UpdateThirdLevelFandI {

    private final IssueIndexingService issueIndexingService;
    public UpdateThirdLevelFandI(IssueIndexingService issueIndexingService) {
        this.issueIndexingService = issueIndexingService;
    }

    /**
     * REST method that updates product at third level to F&I 2.0 if first level is F&I 2.0
     * and updates product at third level to F&I 1.0 if first level is F&I Solutions (F&I).
     *
     */
    @GET
    @Path("/updateFandIThirdLevel")
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateThirdLevelProductForFandI(@QueryParam("jqlQuery") String jqlQuery) throws Exception {
        return Response.ok(updateThirdLevelProductForFandIForIssues(jqlQuery)).build();
    }


    public List<SolutionGroupThirdLevelProdModel> updateThirdLevelProductForFandIForIssues(String jqlQuery) {
        Logger log = LoggerFactory.getLogger(UpdateThirdLevelFandI.class);
        List<SolutionGroupThirdLevelProdModel> solGroupIssueUpdateList = new ArrayList<>();

        List<Issue> issues = getIssuesForProject(jqlQuery); //Use this finally
        log.info("TOTAL NUMBER OF ISSUES CONSIDERED FOR UPDATING THIRD LEVEL IS " + issues.size());
        ...
        for (Issue currentissue : issues) {
            ...
            if (solutionGroupProdValuesList.size() == 2 &&
                    solutionGroupProdValuesList.get(0).equals("F&I Solutions (F&I)")) {
                solutionGroupProdValuesList.add(2, "F&I 1.0");
            } else if (solutionGroupProdValuesList.size() == 2 &&
                    solutionGroupProdValuesList.get(0).equals("F&I 2.0")) {
                solutionGroupProdValuesList.add(2, "F&I 2.0");
            } else {
                log.info("Solution group has to be either F&I Solutions (F&I) or F&I 2.0, and there must be a product" +
                        " for this script to run for issue (also it can be that this issue is already updated at third level?) :"
                        + currentissue.getKey());
                continue;
            }
            ...
            if (solutionGroup != null) {
                ...
                if (productEnable == null) {
                    log.info("THIS ISSUE IS NOT UPDATED AT THIRD LEVEL BECAUSE PRODUCT IS NOT ENABLED "
                            + currentissue.getKey());
                    continue;
                }
                ...
                if (subProductEnable == null) {
                    log.info("THIS ISSUE IS NOT UPDATED AT THIRD LEVEL BECAUSE THIRD LEVEL F&I SUB-PROD IS NOT ENABLED "
                            + currentissue.getKey());
                    continue;
                }

                log.debug("$$$$$$$$$$$$$$$$$$ issueMap third level f and i $$$$$$$$$$$$$$"
                        + cascadeList);
                ...
                try {
                    ...
                    log.error("THIS ISSUE COULD NOT BE UPDATED FOR THIRD LEVEL F & I - PLEASE CHECK " + issue.getKey(), ie);
                }
                ...
                try {
                    ...
                    log.error("THIS ISSUE COULD NOT BE UPDATED FOR THIRD LEVEL F & I - PLEASE CHECK " + currentissue.getKey(),ie);
                }
            ...
            String subProductForJSON = valuesListNew.size() > 2 ? valuesListNew.get(2).toString() : "";
            SolutionGroupThirdLevelProdModel bean = new SolutionGroupThirdLevelProdModel(solutionGroupForJSON,
                    productForJSON,subProductForJSON, currentissue.getKey());
            solGroupIssueUpdateList.add(bean);
        }

        log.info("NUMBER OF ISSUES UPDATED WITH THE THIRD LEVEL F & I value SUCCESSFULLY " + issuesUpdatedCount);
