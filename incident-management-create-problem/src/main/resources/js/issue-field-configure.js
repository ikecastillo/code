var Mapper = {
    addMapping: function() {
        ...
        if(AJS.$("#selectedDefaultIssueFields").val()!=='' &&  AJS.$("#mappingfield").val()!==''){
            ...
            AJS.$.ajax({
                url :"/rest/dt/fieldsmap/1.0/IssueFieldMap/addIssueFieldMapping.json?jiraField="+AJS.$("#selectedDefaultIssueFields").val()+"&mappingField="+AJS.$("#mappingfield").val()+"&fromIssueType="+AJS.$("#selectedProjectIssueTypes").val()+"&toIssueType="+AJS.$("#selectedIssueTypes").val(),
                type: "PUT",
	...
	removeMapping:function(jiraField, mappingField,fromIssueType,toIssueType) {
       AJS.$.ajax({
                url :"/rest/dt/fieldsmap/1.0/IssueFieldMap/deleteIssueFieldMapping.json?jiraField="+jiraField+"&mappingField="+mappingField+"&fromIssueType="+fromIssueType+"&toIssueType="+toIssueType,
                type: "PUT",
                contentType: 'application/json',
                success: function (response) {
