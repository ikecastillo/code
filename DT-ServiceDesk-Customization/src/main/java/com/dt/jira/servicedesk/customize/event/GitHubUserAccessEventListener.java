...
/**
 * GitHubUserAccessEventListener.java event listener class
 * In create event it .
 * When subtask get approved then adds user to dtinc ldap group i.e GitHubMembers
 * 
 */

public class GitHubUserAccessEventListener implements InitializingBean,
		DisposableBean {
	...
	private final Logger logger = Logger
			.getLogger(GitHubUserAccessEventListener.class);
	...
	private final String CUSTOM_FIELD_DTINC_LDAP_EXCEPTION_MESSAGE="DTINC LDAP Exception Message";
	...
	private final String ISSUE_TYPE_GITHUB_USER_ACCESS_REQUEST="GitHub User Access Request";


	public GitHubUserAccessEventListener(EventPublisher eventPublisher,
	...
	public void onIssueEvent(IssueEvent issueEvent) {
		try {
			...
			logger.info(" ------------------------------------onIssueEvent starts-----------------------------------------");
			logger.info(" eventTypeId---"+ eventTypeId);
			logger.info(" Issue Parent---"+ issueParent);
			logger.info(" ------------------------------------+++++++++++++++++++++-----------------------------------------");

			logger.info(issueParent.getStatusObject().getId()+"-------GitHubUserAccessEventListener status -------------"+issueParent.getStatusObject().getName());
			/* Added condition for only GitHubUserAccess issue type*/
			if (eventTypeId.equals(EventType.ISSUE_CREATED_ID) && issueParent.getIssueType().getName().equalsIgnoreCase(ISSUE_TYPE_GITHUB_USER_ACCESS_REQUEST)) {
				logger.info("---------------------------GitHubUserAccessEventListener  Executed issue created ---------------------");
				//createSubtasksByConfiguration(issueEvent);
			} else if(issueParent.getIssueType().getName().equalsIgnoreCase(ISSUE_TYPE_GITHUB_USER_ACCESS_REQUEST)){
				logger.info("  GitHubUserAccessEventListener EventType.ISSUE_GENERICEVENT_ID");
				...
				logger.info(" GitHubUserAccessEventListener Executed issue update");
				...
				if (issueParent.getStatusObject().getName()
					...
					String userDn = getDistinguishedNamebyUserName(issueParent
							.getReporterId());
					logger.info(project.getName()+"GitHubUserAccessEventListener----projectName------portals-----------"+issueParent.getIssueTypeObject()
							.getName());
					...
					for (String group : groups) {
						try {
							logger.info("--------------------------++++++++++++++++++++++++++++++++++++++++++----------------------------");
							logger.info("group name is ==="+group);
							logger.info("--------------------------++++++++++++++++++++++++++++++++++++++++++----------------------------");
	...
	private void updateIssueTextField(MutableIssue issue,String newvalue){
		try{
		logger.info("GitHubUserAccessEventListener - -- +"+ newvalue);
			String existingValue = "";
		JiraAuthenticationContext  authenticationContext  = ComponentAccessor.getJiraAuthenticationContext();
		CustomFieldManager cfManager = ComponentAccessor.getCustomFieldManager();
		CustomField customFeildObj = cfManager.getCustomFieldObjectByName(CUSTOM_FIELD_DTINC_LDAP_EXCEPTION_MESSAGE);
		customFeildObj.updateValue(null, issue, new ModifiedValue(
	...
	public String getDistinguishedNamebyUserName(String username) throws NamingException,Exception{
		...
		try {
			...
			String ldapBaseDn = "dc=dt,dc=inc" ;//(String) settings.get(ServiceDeskLdap.class.getName() + ".ldapBaseDn");
	...
	private DirContext getLDAPContext() throws NamingException {
		...
		String ldapInitCtx = "com.sun.jndi.ldap.LdapCtxFactory";  //(String) settings.get(ServiceDeskLdap.class.getName() + ".ldapInitCtx");
	     // Get LDAP URL
		String ldapSrvrName = "10.134.38.10:389";  //(String) settings.get(ServiceDeskLdap.class.getName() + ".ldapSrvrName");
		// Get LDAP BaseDN  Name
		String ldapBaseDn = "dc=dt,dc=inc"; //(String) settings.get(ServiceDeskLdap.class.getName() + ".ldapBaseDn");
		// Get LDAP user login Name
		String ldapUid = "cn=DTJira Admin,ou=Service Accounts,ou=Lake Success,dc=dt,dc=inc";  //(String) settings.get(ServiceDeskLdap.class.getName()+ ".ldapUid");
		String ldapPwd = (String) settings.get(ServiceDeskLdap.class.getName()+ ".dtincLdapPwd");
